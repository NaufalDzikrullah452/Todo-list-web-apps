 <!--**********************************
            Sidebar start
        ***********************************-->
        <div class="nk-sidebar">           
            <div class="nk-nav-scroll">
                <ul class="metismenu" id="menu">
                    <li class="nav-label">Dashboard</li>
                    <li>
                        <a href="<?= site_url('index.php/dashboard/today') ?>" aria-expanded="false">
                            <i class="fa fa-calendar-o menu-icon"></i><span class="nav-text">Today</span>
                        </a>
                    </li>
                    <li>
                        <a href="<?= site_url('index.php/dashboard/priority') ?>" aria-expanded="false">
                            <i class="fa fa-star-o menu-icon"></i><span class="nav-text">Priority</span>
                        </a>
                    </li>
                    <li>
                        <a href="<?=site_url('index.php/dashboard/task') ?>" aria-expanded="false">
                            <i class="icon-list menu-icon"></i><span class="nav-text">Task</span>
                        </a>
                    </li>
                    <li>
                        <a href="<?= site_url('index.php/dashboard/performance') ?>" aria-expanded="false">
                            <i class="icon-graph menu-icon"></i><span class="nav-text">Performance</span>
                        </a>
                    </li>
                </ul>
            </div>
        </div>
        <!--**********************************
            Sidebar end
        ***********************************-->